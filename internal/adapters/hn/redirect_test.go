...
func TestRedirectAllowlist(t *testing.T) {
	t.Run("allowed host passes", func(t *testing.T) {
		if err := redirectAllowlist(mustReq(t, "https://hn.algolia.com/api"), nil); err != nil {
			t.Errorf("allowed host rejected: %v", err)
		}
	})
	t.Run("disallowed host rejected", func(t *testing.T) {
		err := redirectAllowlist(mustReq(t, "https://evil.example.com/"), nil)
	...
	t.Run("too many hops rejected", func(t *testing.T) {
		via := []*http.Request{{}, {}, {}}
		if err := redirectAllowlist(mustReq(t, "https://news.ycombinator.com/"), via); err == nil {
...
func TestIsCrossDomainRedirectErr(t *testing.T) {
	...
	if !isCrossDomainRedirectErr(errors.New("hn: cross-domain redirect rejected: x")) {
