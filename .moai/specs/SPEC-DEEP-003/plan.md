...
- B3: B1 GREEN — `errgroup.WithContext` + BFS queue 구현. mock
  Researcher interface로 LLM/fanout dependency 격리.
- B4: REFACTOR — 코드 정리.
...
- D3: `deploy/postgres/migrations/{NN}_deep_runs.up.sql` + `down.sql`
  신규(repo 표준 디렉토리 — 기존 `0001_create_docs.sql` precedent).
  `{NN}` 시퀀스 번호와 migration 도구 핀은 spec.md §8 Open Questions
  참조 — Run phase 진입 전 별도 확정.
...
| §5.5 Crash recovery reclassify | `TestPersistenceReclassifyOnReload` | `internal/deepagent/persistence_test.go` |
...
| Postgres migration drift | Low | Medium | `deploy/postgres/migrations/{NN}` versioning(sequence/tool은 spec.md §8 OQ-1로 미해결 보류) + `down.sql` 짝 보장 (Phase D3) |
...
- [ ] 모든 REQ-DEEP3-001..012가 적어도 하나의 테스트 함수로 cover됨
- [ ] 모든 NFR-DEEP3-001..008이 quantitative assertion으로 검증됨
