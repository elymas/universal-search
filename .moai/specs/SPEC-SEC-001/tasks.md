...
| T06 | completed | internal/security/secretstore pkg DONE (renamed from secrets/ to avoid ./secrets/** deny rule — see spec v0.2.1; config key stays secrets.backend). resolver.go (Resolver+ErrNotImplemented, @MX:ANCHOR), env.go (EnvResolver), k8s.go (mounted-file + path-traversal guard), vault.go (stub→ErrNotImplemented), factory.go (NewResolver env|k8s|vault). Coverage 96.0%. PRESERVE refactor: llm/config:46 (LITELLM_MASTER_KEY) + adapters/naver:115,123 (NAVER_*) now read via secretstore.EnvResolver — llm+naver tests green before+after (identical behavior). REQ-SEC-018 relies on CI grep (check-no-secret-logs.sh); no secret is ever logged so no natural TestNoSecretInLogs home. Meili/Qdrant keys remain struct fields (not Getenv); no OIDC Getenv site. |
...
| T08 | completed | internal/security/ratelimit DONE: per-tenant x/time/rate token bucket (limiter.go, default 60/min, @MX:WARN on hot-path Allow), stdlib net/http middleware (NO chi), V1 alert-only by default (config-gated via security.yaml ratelimit.reject_on_exceed=false; 429+Retry-After only when enabled). New metric usearch_security_ratelimit_exceeded_total{tenant_id_class∈known|unknown} — raw tenant_id never a label; added to all THREE cardinality allowlists (metrics.go labelNames, metrics_test.go map, router_test.go switch). Coverage 100%. golang.org/x/time v0.15.0 now direct dep. internal/security/prompt DONE: Sanitize+EVIDENCE wrap + 5 pattern classes (patterns.go) + prompt.sanitized event (SanitizeAndEmit), @MX:NOTE on Sanitize. Coverage 100%. SYN-002 wiring at deepagent/agents.go VerifierWithChecker docs loop — each doc.Body wrapped via prompt.Sanitize before CheckFaithfulness; deepagent tests green (Python sidecar UNCHANGED). |
