...
- **Threads / X credentials via the Resolver** — routed 2026-06-24 (2nd pass).
  `cmd/usearch-mcp` `buildProductionRegistryWithResolver` now resolves
  `THREADS_ACCESS_TOKEN` and `X_BEARER_TOKEN` through `secretstore.Resolver`
  and registers them with `SkipAuthCheck: true`; the raw `os.Getenv` reads for
  those secrets were removed. Verified by `TestThreadsRegisteredViaResolver` /
  `TestXBearerViaResolver`. (Originally out-of-scope for SEC-002; folded in.)
