...
- 2026-05-21: status draft → planned. tasks.md generated (62 tasks across M1-M7 milestones: 23 RED, 23 GREEN, 8 REFACTOR, with full coverage of 66 plan.md §3 tests and 15 EARS REQs + 4 NFRs). No version bump for status flip — v0.1.2 retained.

- 2026-05-21 (v0.1.2 plan-auditor iter-2 patches, limbowl via manager-spec):
  Second-round plan-auditor review identified 3 BLOCKERs + 7 MAJORs. Applied
  10 targeted patches (3 B + 7 M):
  - P-B1: Renamed metric label value `fail_timeout` → `fail_error` in
    REQ-DEEP2-006, REQ-DEEP2-008, NFR-DEEP2-002, plan.md M6 metrics list,
    and acceptance.md Edge Case 2. Reason: `fail_timeout` was being
    incremented on generic Verifier sidecar 5xx errors (not only on
    timeouts), creating a contract/test mismatch. Renamed to `fail_error`
    to reflect actual semantics.
  - P-B2: Split REQ-DEEP2-009a into 009a-SSE (terminal SSE event,
    HTTP 200 already on wire) and 009a-Buffered (HTTP 503 + JSON body).
    Same split applied to REQ-DEEP2-009b. Acceptance.md Scenario 3 now
    tests both paths explicitly (`TestSSEMaxRetryEmitsTerminalEvent` vs
    `TestBufferedMaxRetryReturns503`).
  - P-B3: Replaced bare `REQ-DEEP2-009` references with `REQ-DEEP2-009a-SSE`,
    `REQ-DEEP2-009a-Buffered`, `REQ-DEEP2-009b-SSE`, or `REQ-DEEP2-009b-Buffered`
    per context (REQ-DEEP2-007, NFR-DEEP2-001, plan.md test #44, MX tag
    plan, etc.). Combined with P-B2, EARS REQ count is now 15 (was 13
    in v0.1.1 — net +2 from the two-way SSE/Buffered split of both 009a
    and 009b).
  - P-M1: Added explicit `at_agent` semantics to RDC-3 and REQ-DEEP2-007
    pipeline_cancelled payload: denotes the next agent that would have
    been invoked; if cancellation occurs during an in-progress agent,
    the in-progress agent name is used.
  - P-M2: Reworded NFR-DEEP2-001 to define Go-side orchestration overhead
    (p95 ≤ 1s, 50-iteration mocked statistical test) separately from
    end-to-end prod p95 (≤ 60s on staging corpus 20-50 docs, verified
    in /moai sync smoke test). Added DOD item to acceptance.md §1.
  - P-M3: REQ-DEEP2-012 (empty corpus) now requires Researcher to skip
    its LLM invocation when fanout returns empty and to return
    IsEmpty:true immediately. Histogram outcome remains `success`,
    SSE agent_completed outcome field uses `empty_corpus` (label vs
    field naming clarification added).
  - P-M4: Fixed typo `TestEmptyFanoutOutconeCounterIncrements` →
    `TestEmptyFanoutOutcomeCounterIncrements` in acceptance.md.
  - P-M5: Renumbered TDD test catalog 23a → 24 and shifted 24-57 → 25-58.
    Updated total test count 57 → 58 in plan.md.
  - P-M6: Replaced "byte-identical" with "schema-identical AND
    semantically equivalent" in REQ-DEEP2-011 and NFR-DEEP2-003. Same
    event types in same order, same field names per event, same field
    types. Non-deterministic fields (request_id, timestamps, duration_ms,
    latency_ms, cost_usd) MAY differ in value.
  - P-M7: Defined explicit `streamsynth.LongFormSource` interface that
    both `deepreport.Report` and `deepagent.WriterDraft` implement.
    Added to plan.md M3 + M5 task lists and File Impact Map.
  Version 0.1.1 → 0.1.2. SPEC status remains `draft` (flip to `approved`
  happens after tasks.md generation).
...
- 2026-05-21 (initial draft v0.1.0, limbowl via manager-spec):
  ...
  13 EARS REQs (11 × P0 + 2 × P1, after 009a/009b split in v0.1.1), 4 NFRs, ≤5 modules(Endpoint /
...
- **RDC-3 (Context cancellation)** — (a) Resolved. Orchestrator는
  ...
  `pipeline_cancelled` payload의 `at_agent` 필드 semantics:
  cancellation 감지 시점에 다음으로 호출될 예정이었던 agent의 이름을
  사용한다. 만약 cancellation이 어떤 agent의 in-progress 실행 도중
  감지되면 (`ctx.Done()`이 LLM call mid-flight에서 fire), in-progress
  중인 그 agent의 이름을 사용한다.
...
- **RDC-9 (Error propagation)** — (a) Resolved. REQ-DEEP2-003은
  ...
  최대 시도 횟수까지 누적된 max-retry exhaustion은 두 REQ가
  관할한다: SSE 스트림 활성 시 REQ-DEEP2-009a-SSE (terminal SSE
  event, HTTP 200 already on wire), buffered 응답 시 REQ-DEEP2-009a-Buffered
  (HTTP 503 + JSON body). Researcher/Reviewer/Verifier 자체의 비복구성
  에러(timeout, LLM 오류 등)도 같은 패턴으로 분리된다:
  REQ-DEEP2-009b-SSE / REQ-DEEP2-009b-Buffered. 네 개의 unwanted REQ는
  공통적으로 (a) `pipeline_failed` SSE 또는 HTTP 503을 emit, (b) Writer
  retry trigger 금지, (c) `usearch_deep_outcomes_total{outcome="error_pipeline_failed"}`
  를 1회 증가시키는 동작을 공유하지만, 스트림 상태와 IF precondition에 따라
  HTTP 응답 메커니즘이 결정된다. 각 REQ는 별개의 테스트 그룹을 갖는다.
...
| [NEW/MODIFY] | `internal/streamsynth/longform_source.go` | NEW interface `LongFormSource` defining a contract for objects walkable by `StreamLongFormReport` (`SectionCount() int`, `Section(i int) Section`, `Citations() []Citation`, `Metadata() ReportMetadata`). Both `deepreport.Report` (DEEP-001) and `deepagent.WriterDraft` (DEEP-002) implement this interface. Removes the need for streamsynth to know either concrete type. |
...
2. **Pipeline** — Agent ordering, retry loop, cancellation, empty corpus (REQ-002, 003, 005, 009a-SSE, 009a-Buffered, 009b-SSE, 009b-Buffered, 012)
...
| REQ-DEEP2-011 | Ubiquitous | The `mode=storm` route SHALL preserve SPEC-DEEP-001 behavior in a **schema-identical AND semantically equivalent** form: same SSE event types in same order, same field names per event, same field types per field. Non-deterministic fields (`request_id`, timestamps, `duration_ms`, `latency_ms`, `cost_usd`) MAY differ in value across runs but SHALL retain identical names, types, and presence. The `/deep?mode=agents` and `/deep?mode=storm` code paths SHALL share NO mutable state — no global variables, no shared singletons that mutate across requests. DEEP-001 acceptance suite SHALL remain 100% green after DEEP-002 lands (regression check). When `?mode=` is absent, the handler SHALL default to `mode=storm` to preserve backward compatibility with pre-DEEP-002 clients. | P0 | `TestDeepHandlerStormModeUnchanged` (regression via DEEP-001 acceptance suite), `TestDeepHandlerModeAbsentDefaultsToStorm`, `TestDeepHandlerNoSharedMutableStateBetweenModes`, `TestStormModeResponseSchemaIdenticalPrePostDeep002` |
...
| REQ-DEEP2-009a-SSE | Unwanted | IF Writer is invoked the maximum allowed times (`DEEP_AGENT_MAX_RETRIES + 1`, default 3) AND Verifier still returns `uncited_sentences_count > 0` after the final attempt AND the SSE stream is already active (response headers have been flushed with `Content-Type: text/event-stream`), THEN the orchestrator SHALL abort the pipeline AND the handler SHALL emit a terminal SSE event `pipeline_failed{failed_agent: "writer", reason: "verifier_rejection_exhausted", attempts: <N>, uncited_count: <N>, retry_count: <N-1>}` and close the stream cleanly. HTTP status remains 200 (already on the wire — SSE protocol forbids retroactive status code changes after headers are flushed). NO HTTP status code change SHALL be attempted in this case. The counter `usearch_deep_agent_verifier_gate_results_total{result="fail_uncited"}` SHALL increment for each Verifier rejection (including the final one), AND `usearch_deep_outcomes_total{outcome="error_pipeline_failed"}` SHALL increment exactly once. | P0 | `TestSSEMaxRetryEmitsTerminalEvent`, `TestSSEMaxRetryHttpStatusStays200`, `TestErrorOutcomeCounterIncrementsExactlyOnce` |
| REQ-DEEP2-009a-Buffered | Unwanted | IF Writer is invoked the maximum allowed times AND Verifier still returns `uncited_sentences_count > 0` after the final attempt AND the response is buffered (`?stream=false` or Accept header does not advertise `text/event-stream` — response headers NOT yet flushed), THEN the handler SHALL return HTTP 503 with `Content-Type: application/json` body `{"error": "pipeline_failed", "detail": "Writer exhausted <N> attempts; Verifier still rejected", "uncited_count": <N>, "attempts": <N>, "retry_count": <N-1>}`. The same Prometheus counter increments as REQ-DEEP2-009a-SSE apply. | P0 | `TestBufferedMaxRetryReturns503`, `TestBufferedMaxRetryJsonBodyShape`, `TestErrorOutcomeCounterIncrementsExactlyOnce` |
| REQ-DEEP2-009b-SSE | Unwanted | IF Researcher, Reviewer, or Verifier (the agent itself, distinct from Verifier's PASS/FAIL outcome) returns a non-recoverable error (LLM upstream failure, timeout, panic) at any pipeline stage AND the SSE stream is already active (response headers have been flushed), THEN the orchestrator SHALL abort the pipeline AND the handler SHALL emit a terminal SSE event `pipeline_failed{failed_agent: "<name>", reason: "<reason>"}` and close the stream cleanly. HTTP status remains 200 (already on the wire). No Writer retry SHALL be triggered by such errors. The counter `usearch_deep_outcomes_total{outcome="error_pipeline_failed"}` SHALL increment exactly once per aborted pipeline. | P0 | `TestSSEResearcherErrorEmitsTerminalEvent`, `TestSSEReviewerErrorEmitsTerminalEvent`, `TestSSEVerifierErrorEmitsTerminalEvent`, `TestNonVerifierErrorsDoNotTriggerRetry` |
| REQ-DEEP2-009b-Buffered | Unwanted | IF Researcher, Reviewer, or Verifier returns a non-recoverable error AND the response is buffered (headers NOT yet flushed), THEN the handler SHALL return HTTP 503 with `Content-Type: application/json` body `{"error": "pipeline_failed", "detail": "<agent> failed: <reason>", "failed_agent": "<name>"}`. No Writer retry SHALL be triggered. The same Prometheus counter increments as REQ-DEEP2-009b-SSE apply. | P0 | `TestBufferedResearcherErrorReturns503`, `TestBufferedReviewerErrorReturns503`, `TestBufferedVerifierErrorReturns503`, `TestNonVerifierErrorsDoNotTriggerRetry` |
| REQ-DEEP2-012 | Unwanted | IF `fanout.Dispatch()` returns an empty `Result.Docs` slice (zero docs after all adapter dedup), THEN the Researcher agent SHALL SKIP its own LLM invocation (no claim extraction call is made — there is nothing to extract from) and SHALL return `ResearcherOutput{IsEmpty: true}` immediately. The orchestrator SHALL short-circuit the pipeline: Reviewer/Writer/Verifier SHALL NOT be invoked. The response SHALL be HTTP 200 with body `{"final": {"sections": [], "citations": []}, "agent_log": [{"agent": "researcher", "outcome": "empty_corpus"}], "schema_version": 1}`. The counter `usearch_deep_outcomes_total{outcome="empty_corpus"}` SHALL increment exactly once. **Histogram label vs SSE field naming clarification**: the `usearch_deep_agent_duration_seconds{agent="researcher", outcome=...}` Prometheus label observes value `success` for the empty-fanout case (Researcher completed normally without error — the bounded label set `{success, error}` from NFR-DEEP2-002 contains no `empty_corpus` value). The SSE `agent_completed.outcome` JSON field, by contrast, MAY carry the unbounded string `empty_corpus` because the SSE payload is not a Prometheus label and has no cardinality constraint. The SSE stream SHALL emit `agent_started{agent: "researcher"}`, then `agent_completed{agent: "researcher", outcome: "empty_corpus"}`, then a terminal `done` event with `total_sections: 0` — no `pipeline_failed` event (empty corpus is a degenerate but non-error outcome). | P0 | `TestEmptyFanoutShortCircuitsPipeline`, `TestEmptyFanoutResponseShape`, `TestEmptyFanoutOutcomeCounterIncrements`, `TestEmptyFanoutSSEEventSequence`, `TestEmptyFanoutResearcherSkipsLLMInvocation`, `TestEmptyFanoutHistogramOutcomeIsSuccess` |
...
| ID | Name | Requirement |
|----|------|-------------|
| NFR-DEEP2-001 | Performance (orchestration overhead + prod p95) | Two separate budgets apply: **(a) Go-side orchestration overhead p95 ≤ 1 s** when LLM and faithfulness sidecar calls are mocked — this isolates the orchestrator/SSE/metrics overhead from upstream LLM latency. Verified via a 50-iteration statistical test in `internal/deepagent/orchestrator_test.go` (TestE2ELatencyP95Under1SecondMocked). **(b) End-to-end prod p95 ≤ 60 s** measured from `cmd/usearch-api` request acceptance to terminal SSE event (`done` on success, `pipeline_failed` on failure, `pipeline_cancelled` on cancel) for a corpus of 20–50 fanout docs on the staging corpus, with Verifier passing on the first or second attempt and default model tiers (Researcher/Reviewer Haiku, Writer/Verifier Sonnet). Budget (b) is verified by a smoke test executed during `/moai sync` against the staging deployment (NOT a unit test). 모델 별칭이 더 큰 모델로 오버라이드된 경우 또는 max-retry exhaustion 시나리오는 본 NFR의 budget violation으로 간주하지 않으며 REQ-DEEP2-009a-SSE / REQ-DEEP2-009a-Buffered / REQ-DEEP2-009b-SSE / REQ-DEEP2-009b-Buffered의 degraded path로 처리된다. DEEP-001의 NFR-DEEP1-001 (p50≤180s, p95≤300s)와 별도의 예산이며 DEEP-002의 multi-agent 오버헤드 + 재시도 가능성을 반영해 더 짧게 설정되었다(no STORM full-article generation, 짧은 합성 출력). |
| NFR-DEEP2-002 | Cardinality Safety | All Prometheus label values introduced by this SPEC SHALL come from bounded enumerable sets pre-declared at startup time, per NFR-OBS-002 from SPEC-OBS-001. Specifically: `agent ∈ {researcher, reviewer, writer, verifier}`, `outcome ∈ {success, error}`, `result ∈ {pass, fail_uncited, fail_error}`. Enum-like Go types (e.g., `type Agent string` with `const` declarations) SHALL enforce the bounded set at compile time within `internal/deepagent/types.go`. No user-derived strings (query content, model name override, adapter name, error message snippet) SHALL appear in label position. The existing cardinality guard test in `internal/obs/metrics/metrics_test.go` SHALL remain green without label-NAME allowlist amendments (only new label NAMES would require allowlist updates; DEEP-002 adds new VALUES to label names that may be pre-existing OR adds new label names that the test framework validates as bounded). |
| NFR-DEEP2-003 | Backward Compatibility | SPEC-DEEP-001 acceptance suite (`services/storm/tests/`, `internal/deepreport/`, `internal/streamsynth/longform_test.go`, `cmd/usearch-api/handlers/synthesis.go` regression) SHALL remain 100% green after DEEP-002 lands. The `/deep?mode=storm` request path SHALL produce **schema-identical AND semantically equivalent** responses pre- and post-DEEP-002: same SSE event types in same order, same field names per event, same field types per field. Non-deterministic field values (`request_id`, timestamps, `duration_ms`, `latency_ms`, `cost_usd`) MAY differ across runs but field names, types, and presence SHALL be identical. The `?mode=` query parameter SHALL default to `storm` when absent. No shared mutable global state SHALL be introduced between the `storm` and `agents` code paths (separate handler functions, separate orchestrator entry points, separate metric collectors). Code review SHALL verify that `internal/deepagent/` imports do not include `internal/deepreport/` or `services/storm/`. |
...
| Scenario 3 — Max-retry exhaustion (Verifier rejects all 3 attempts → 503 or terminal SSE) | REQ-009a-SSE, 009a-Buffered, 007, 008 |
...
| Scenario 8 — Researcher error aborts pipeline | REQ-009b-SSE, 009b-Buffered |
...
- **Multi-agent retry coordination beyond Writer** — Researcher/Reviewer/Verifier의 retry는 본 SPEC이 지원하지 않는다.
  이들 agent의 transient error는 즉시 pipeline abort + terminal SSE event
  (REQ-DEEP2-009b-SSE) 또는 HTTP 503 (REQ-DEEP2-009b-Buffered)로 surfacing된다.
